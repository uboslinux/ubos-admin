...
sub instantiateAppConfigurationItem {
    ...
    if( 'file' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::File->new( $json, $appConfig, $installable );
    } elsif( 'directory' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::Directory->new( $json, $appConfig, $installable );
    } elsif( 'directorytree' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::DirectoryTree->new( $json, $appConfig, $installable );
    } elsif( 'symlink' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::Symlink->new( $json, $appConfig, $installable );
    } elsif( 'perlscript' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::Perlscript->new( $json, $appConfig, $installable );
    } elsif( 'sqlscript' eq $type ) {
        $ret = IndieBox::AppConfigurationItems::Sqlscript->new( $json, $appConfig, $installable );
    } else {
        if( $type =~ m!^(.*)-database$! ) {
            ...
            if( $role ) {
                $ret = IndieBox::AppConfigurationItems::GenericDatabase->new( $role->name, $json, $appConfig, $installable );
            }
        }
    }
    unless( $ret ) {
