...
my $APPCONFIGPARSDIR = '/var/lib/indiebox/appconfigpars';
...
sub _deployOrCheck {
    ...
    my @rolesOnHost = IndieBox::Host::rolesOnHostInSequence();
...
sub _undeployOrCheck {
    ...
    my @reverseRolesOnHost = reverse IndieBox::Host::rolesOnHostInSequence();
...
sub _initialize {
    ...
    if( $self->{json}->{accessoryids} ) {
        my @acc = map { new IndieBox::Accessory( $_ ) } @{$self->{json}->{accessoryids}};
