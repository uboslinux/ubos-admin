#!/usr/bin/perl
#
# The mechanism by which we allocate ports.
#
...
sub checkInstallableManifestForRole {
    ...
    if( $jsonFragment->{installers} ) {
        $installable->myFatal( "roles section: role $roleName: installers not allowed here" );
    }
    if( $jsonFragment->{upgraders} ) {
        $installable->myFatal( "roles section: role $roleName: upgraders not allowed here" );
    }
    ...
    $self->SUPER::checkManifestForRoleGenericAppConfigItems( $roleName, $installable, $jsonFragment, $allowedTypes, $retentionBuckets, $skipFilesystemChecks, $config );
}
