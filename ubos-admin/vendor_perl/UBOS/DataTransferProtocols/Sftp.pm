...
sub parseLocation {
    ...
    if( $uri->authority() !~ m!\S+\@\S+! ) {
        fatal( 'Need to provide user info in the URL, e.g. sftp://joe@example.com/destination' );
    ...
    if( $sshOptions ) {
        $dataTransferConfig->setValue( 'sftp', $uri->authority(), 'sshoptions', $sshOptions );
    }
    if( $limit ) {
        unless( $limit =~ m!^\d+$! ) {
...
sub send {
    ...
    my $sshOptions = $dataTransferConfig->getValue( 'sftp', $uri->authority(), 'sshoptions' );
    if( $sshOptions ) {
