...
sub createDiskLayout {
    ...
    # Option 1: a single image file (invalid -- don't know how to dd to disk image, well, at least not without complications)
    ...
    if( $directory ) {
        ...
        if( @$argvp == 1 ) {
            ...
            if( $ret && UBOS::Install::AbstractDiskLayout::isFile( $rootDiskOrImage )) {
                # Option 1
                error( 'Invalid invocation: cannot install to a disk image for this device class.' );
                $ret = undef;
